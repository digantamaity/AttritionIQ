...
<style>
    ...
    .stNumberInput, .stSelectbox, .stSlider, .stFileUploader {
        margin-bottom: 16px;
    }
</style>
...
with st.sidebar:
    ...
    if data_source == "Sample IBM HR Dataset":
        # Hardcoded filters for better UX on default dataset
        ...
        # Fully Dynamic Filters for Custom Dataset
        if cat_cols:
...
if selected == "Home":
    ...
    if data_source == "Sample IBM HR Dataset":
        ...
        with col1:
            st.markdown(f'''<div class="glass-card glass-card-accent-1"><div class="metric-title">Total Employees</div><div class="metric-value">{total_emp}</div></div>''', unsafe_allow_html=True)
        with col2:
            st.markdown(f'''<div class="glass-card glass-card-accent-2"><div class="metric-title">Attrition Rate</div><div class="metric-value">{attrition_rate:.1f}%</div></div>''', unsafe_allow_html=True)
        with col3:
            st.markdown(f'''<div class="glass-card glass-card-accent-3"><div class="metric-title">Average Age</div><div class="metric-value">{avg_age:.1f} yrs</div></div>''', unsafe_allow_html=True)
        with col4:
            st.markdown(f'''<div class="glass-card glass-card-accent-4"><div class="metric-title">Avg Monthly Income</div><div class="metric-value">${avg_salary:,.0f}</div></div>''', unsafe_allow_html=True)
    else:
        # Generic KPIs for Custom Uploads
        ...
        with col1:
            st.markdown(f'''<div class="glass-card glass-card-accent-1"><div class="metric-title">Total Rows</div><div class="metric-value">{total_rows}</div></div>''', unsafe_allow_html=True)
        with col2:
            st.markdown(f'''<div class="glass-card glass-card-accent-2"><div class="metric-title">Total Columns</div><div class="metric-value">{total_cols}</div></div>''', unsafe_allow_html=True)
        with col3:
            st.markdown(f'''<div class="glass-card glass-card-accent-3"><div class="metric-title">Categorical Features</div><div class="metric-value">{num_cat}</div></div>''', unsafe_allow_html=True)
        with col4:
            st.markdown(f'''<div class="glass-card glass-card-accent-4"><div class="metric-title">Total Missing Values</div><div class="metric-value">{missing_vals}</div></div>''', unsafe_allow_html=True)
    ...
    if data_source == "Sample IBM HR Dataset":
        ...
        # Dynamic Generic Analytics for Custom Uploads
...
elif selected in ["ML Models", "Live Prediction", "Insights"]:
    if data_source != "Sample IBM HR Dataset":
        st.markdown(f'<div class="main-title">{selected}</div>', unsafe_allow_html=True)
        st.warning("⚠️ **Dataset Compatibility Notice**")
        st.markdown("""
        The Machine Learning engine, Live Prediction tool, and Business Insights are explicitly pre-trained and formulated based on the structure of the **Sample IBM HR Dataset**. 
        
        To access these features, please switch the **Data Source** in the sidebar back to **Sample IBM HR Dataset**.
        """)
    else:
        if selected == "ML Models":
